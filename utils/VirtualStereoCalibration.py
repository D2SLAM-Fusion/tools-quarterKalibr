...
import VirtualStereoUtils.config_loader as config_loader
import VirtualStereoUtils.photometric_calibration as photometricCalibration
import VirtualStereoUtils.split_image as split_image
...
def calibrate_virtual_stereo(input_bag, fov, width, height, fisheye_config, output_path, step,photometric_calibration= None , verbose = False):
    if not os.path.exists(fisheye_config):
        print("[INPUT ERROR]{} fisheye calibration parameters not exist".format(fisheye_config))
        return
    else:
        stereo_gens, _ = config_loader.LoadFisheyeParameter(fisheye_config, fov = fov, width=width, height=height)
    if output_path != "":
        if not os.path.exists(output_path):
            os.makedirs(output_path)
            print("copy aprilgrid.yaml to output folder")
            print(f"{output_path} created")
        aprilgrid_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "april_6x6.yaml")
    ...
    if photometric_calibration:
        print("Loading photometric calibration images from path:", photometric_calibration)
        for i in range(4):
            vig_png_name = photometric_calibration +  f"/cam_{i}_vig_mask.png"
        ...
        for topic, msg, t in bag.read_messages():
            try:
                if topic == "/arducam/image/compressed" or topic == "/arducam/image/raw" \
                    ...
                    calibed = photometricCalibration.calibPhotometricImgsIndividual(imgs, photometrics, is_rgb=False)
    ...
    try:
        ...
        for gen in stereo_gens:
            ...
            pool.apply_async(calibration_task, args=(gen, stereo_gens, output_bag_name, height, width, verbose,), error_callback=print)
        pool.close()
        pool.join()
    except KeyboardInterrupt:
        # Catch Ctrl+C
